function d = finddelay2(x,y,varargin)
%FINDDELAY2 Estimates delays between signals.
%   D = FINDDELAY2(X,Y), where X and Y are row or column vectors of length
...
%   FINDDELAY2(X,Y) returns an estimate of the delay via crosscorrelation.
...
%
%   D = FINDDELAY2(X,Y), where X is a matrix of size MX-by-NX (MX>1, NX>1)
...
%
%   D = FINDDELAY2(...,MAXLAG), uses MAXLAG as the maximum correlation
...
% 			  0 0 0 0 ];
%       MAXLAG = [2 3 2 1];
%       D = finddelay2(X,Y,MAXLAG)
%
%   See also XCORR, ALIGNSIGNALS.

%   based on findelay included Matlab2016a
