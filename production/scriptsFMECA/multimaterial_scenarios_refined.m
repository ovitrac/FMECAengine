...
outputpathref = arrayfun(@(isim) fullfile('multilayer_scenarios_refined',sprintf('reference%d',isim)),1:nsim,'UniformOutput',false);  % folder that contains all reference outputs
% with deletions (relatively to local): 'multilayer_scenarios\omission1'    'multilayer_scenarios\omission2 ....
outputpathdel = arrayfun(@(isim) fullfile('multilayer_scenarios_refined',sprintf('omission%d',isim)),1:nsim,'UniformOutput',false); % folder that contains all omission outputs
figurefolder = fullfile(local,'multilayer_scenarios_refined\figures'); if ~exist(figurefolder,'dir'), mkdir(figurefolder); end
fmecadbfile = 'fmecamodal.mat'; % base of conditions

% Solver parameters
nmesh = 500;   % refined 500
RelTol = 1e-6; % refined 1e-6
AbsTol = 1e-6; % refined 1e-6
