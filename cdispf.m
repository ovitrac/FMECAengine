function count = cdispf(style,format,varargin)
% cdispf displays styled formatted text in the Command Window
%
% Syntax:
%    count = cdispf(style,format,...)
%
% Description:
%    cdispf processes the specified text using the exact same FORMAT
%    arguments accepted by the built-in SPRINTF and FPRINTF functions.
%
%    cdispf then displays the text in the Command Window using the
...
%          only one of them can be used in a single cdispf command. But of
...
%
%    cdispf by itself, without any input parameters, displays a demo
%
% Example:
%    cdispf;   % displays the demo
%    cdispf('text',   'regular black text');
%    cdispf('hyper',  'followed %s','by');
%    cdispf('key',    '%d colored', 4);
%    cdispf('-comment','& underlined');
%    cdispf('err',    'elements\n');
%    cdispf('cyan',   'cyan');
%    cdispf('_green', 'underlined green');
%    cdispf(-[1,0,1], 'underlined magenta');
%    cdispf([1,0.5,0],'and multi-\nline orange\n');
%    cdispf('*blue',  'and *bold* (R2011b+ only)\n');
%    cdispf('string');  % same as fprintf('string') and cdispf('text','string')
%
...
%    <a href="http://undocumentedmatlab.com/blog/cdispf/">http://UndocumentedMatlab.com/blog/cdispf/</a>
%
% Limitations:
%    1. In R2011a and earlier, a single space char is inserted at the
%       beginning of each cdispf text segment (this is ok in R2011b+).
%
%    2. In R2011a and earlier, consecutive differently-colored multi-line
%       cdispfs sometimes display incorrectly on the bottom line.
...
%    2010-06-27: Fix for R2010a/b; fixed edge case reported by Sharron; cdispf with no args runs the demo
...
function [underlineFlag,boldFlag,style,debugFlag] = processStyleInfo(style)
  ...
  if ischar(style)
      ...
      if underlineFlag && boldFlag
          warning('YMA:cdispf:BoldUnderline','Matlab does not support both bold & underline')
      end
  ...
  elseif ~ischar(style)
      error('YMA:cdispf:InvalidStyle','Invalid style - see help section for a list of valid style values')
  ...
  % Style name
      ...
      elseif length(matches) > 1
          error('YMA:cdispf:AmbigStyle','Ambiguous style name - supply extra characters for uniqueness')
...
function showDemo(majorVersion,minorVersion)
  fprintf('cdispf displays formatted text in the Command Window.\n\n');
  fprintf('Syntax: count = cdispf(style,format,...);  click <a href="matlab:help cdispf">here</a> for details.\n\n');
  url = 'http://UndocumentedMatlab.com/blog/cdispf/';
  fprintf(['Technical description: <a href="' url '">' url '</a>\n\n']);
  fprintf('Demo:\n\n');
  boldFlag = majorVersion>7 || (majorVersion==7 && minorVersion>=13);
  s = ['cdispf(''text'',    ''regular black text'');' 10 ...
       'cdispf(''hyper'',   ''followed %s'',''by'');' 10 ...
       'cdispf(''key'',     ''%d colored'',' num2str(4+boldFlag) ');' 10 ...
       'cdispf(''-comment'',''& underlined'');' 10 ...
       'cdispf(''err'',     ''elements:\n'');' 10 ...
       'cdispf(''cyan'',    ''cyan'');' 10 ...
       'cdispf(''_green'',  ''underlined green'');' 10 ...
       'cdispf(-[1,0,1],  ''underlined magenta'');' 10 ...
       'cdispf([1,0.5,0], ''and multi-\nline orange\n'');' 10];
   if boldFlag
       ...
       s = [s 'cdispf(''*blue'',   ''and *bold* (R2011b+ only)\n'');' 10];
       s = strrep(s, ''')',' '')');
       s = strrep(s, ''',5)',' '',5)');
       s = strrep(s, '\n ','\n');
   end
