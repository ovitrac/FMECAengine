function [peakmodel,FitResults,LowestError,BestStart]=peakfit(signal,center,window,NumPeaks,peakshape,extra,NumTrials,start,varargin)
%PEAKFIT performs an iterative least-squares fit of a single Gaussian  peak to the data matrix [x y]
%
% PEAKFIT(signal,center,window);
% Fits a single Gaussian peak to a portion of the matrix "signal".
% The portion is centered on the x-value "center" and has width "window" (in x units).
%
...
% (1=Gaussian (default), 2=Lorentzian, 3=logistic, 4=Pearson, % 5=exponentionally broadened Gaussian)
%
...
% Performs "NumTrials" trial fits and selects the best one (with lowest fitting error).
...
%
% PEAKFIT(signal,center,window,NumPeaks,peakshape,extra,NumTrials,start,'flag',value,'property',value,...)
%   List of implemented flags and properties
%               autozero: remove baseline (default = true)
%               positive: the minimum value used for baseline must be positive (default = false)
%                  mplot: number of plots for function support and plot (default = 1e3)
%                 bounds: relative width used at both ends for determining baselines (defalt = .1);
%
% [peakmodel,FitResults,MeanFitError]=PEAKFIT(signal,center,window...)
%   peakmodel  = stucture coding for the numerical model (for future use and plots)
%       peakmodel.support   = mx1 abscissae where functions are defined (by default m=1000)
%       peakmodel.basisfunc = mxn basis functions (n=number of functions)
%       peakmodel.amplitude = nx1 amplitude of each function
%       peakmodel.offset    = contininuous model of baseline
%   TIP: use plot(peakmodel.support,peakmodel.offset+peakmodel.basisfunc*peakmodel.amplitude,'r-')
%        to plot the corresponding convolved model
%   FitResults = npeaksx4 result array (with baseline removed) with columns
%                1\peak number, 2\peak position, 3\peak height, 4\peak width, 5\peak area
%   MeanFitError = the percent RMS difference between the data and the model in the selected segment
%
%
% T. C. O'Haver (toh@umd.edu).  Version 1.1: April 14, 2009.
% Major Revision INRA\Olivier Vitrac (olivier.vitrac@agroparistech.fr), June 22, 2010
%
%
...
%   21/06/10 reorder data if required
%   22/06/10 major revision to retrieve peakmodel
%   12/09/10 remove printout when ploton is false
%   01/03/12 add default properties
...
% argcheck (revised OV - 01/03/12)
peakfit_options = argcheck(varargin,default);
if length(peakfit_options.bounds)<2, peakfit_options.bounds = peakfit_options.bounds([1 1]); end
ploton = nargout<2;
...
if X(2)<X(1) % additional check by OV
    if ploton, dispf('\tData reordering...'); end
    [X,ind]=sort(X);
    Y = Y(ind);
end
yoffset = min(Y);
if peakfit_options.positive, yoffset = max(0,yoffset); end
Y=Y-yoffset;  % Remove excess offset from data
% Isolate desired segment from data set for curve fitting
% revised OV - 01/03/12
if nargin<2, center = [] ; end
if nargin<3, window = []; end
if isempty(center), center=(max(X)-min(X))/2; end
if isempty(window), window=max(X)-min(X); end
% end revision
...
if peakfit_options.autozero==1,
    X1=min(xx);
    X2=max(xx);
    % revised OV - 01/03/12
    Y1=mean(yy(1:max(1,round(length(xx)*peakfit_options.bounds(1)))));
    Y2=mean(yy((length(xx)-round(length(xx)*peakfit_options.bounds(2))):length(xx)));
    % end revision
    yoffset2 = ((Y2-Y1)/(X2-X1)*(xx-X1)+Y1);
    yy=yy-yoffset2;
    yoffset = yoffset+yoffset2; % baseline segment
end % if

% Define values of any missing arguments
% revised OV - 01/03/12
if nargin<4, NumPeaks=[]; end
if nargin<5, peakshape = []; end
if nargin<6, extra = []; end
if nargin<7, NumTrials = []; end
if nargin<8, start = []; end
if isempty(NumPeaks), NumPeaks=1; end
if isempty(peakshape), peakshape=1; end
if isempty(extra), extra=0; end
if isempty(NumTrials), NumTrials=1; end
if isempty(start), start=calcstart(xx,NumPeaks,xoffset); end
% end revision
...
bestmodel=zeros(yy);
for k=1:NumTrials,
    if ploton, disp(k); end % prints the current trial number as progress indicator
    % newstart
    switch peakshape
        case 1
    ...
    if MeanFitError<LowestError %#ok<BDSCI>
...
AA=zeros(NumPeaks,peakfit_options.mplot);
xxx=linspace(min(xx),max(xx),peakfit_options.mplot);
if length(yoffset)>1
    yyoffset = interp1(xx,yoffset,xxx);
else
    yyoffset = ones(peakfit_options.mplot,1)*yoffset;
end
for m=1:NumPeaks,
    switch peakshape
        case 1
...
if ploton
    ...
    subplot(2,1,1);plot(xx+xoffset,yoffset+yy,'b.'); % Plot the original signal in blue dots
    hold on
    for m=1:NumPeaks,
        plot(xxx+xoffset,yyoffset+height(m)*AA(m,:),'g')  % Plot the individual component peaks in green lines
        area(m)=trapz(xxx+xoffset,height(m)*AA(m,:)); % Compute the area of each component peak using trapezoidal method
    end
    ...
    for marker=1:NumPeaks,
        ...
        subplot(2,1,1);plot([markx+xoffset markx+xoffset],[min(yoffset) max(yoffset)]+[0 max(yy)],'m--')
    end % for
    plot(xxx+xoffset,yyoffset+mmodel,'r');  % Plot the total model (sum of component peaks) in red lines
    hold off;
    axis([min(xx)+xoffset max(xx)+xoffset min(yyoffset)+min(yy) max(yyoffset)+max(yy)]);
...
for m=1:NumPeaks,
    if m==1,
        ...
        FitResults=[FitResults ; [round(m) FitParameters(2*m-1)+xoffset height(m) abs(FitParameters(2*m)) area(m)]]; %#ok<*AGROW>
    end % if
end % for
...
g = (2.*n)./(1+n);
% ----------------------------------------------------------------------
function err = fitlognormal(lambda,t,y) %#ok<*DEFNU>
