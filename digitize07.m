...
%
% ADAVANCED EXAMPLE
%     % digitize07 template to extract data from a figure
%     % INRA\Olivier Vitrac, Yan Zhu, 05/04/2016, 09/06/2017
% 
%{
% STEP1: export XY from top menu bar
% cd('C:\Data\Olivier\Communications\PACK2017\raw')
project = 'Dtoluene_ref1';
if exist(sprintf('%s.mat',project),'file'), load(sprintf('%s.mat',project)), end
digitize07(sprintf('%s.png',project))

% STEP2: the physical range
xreal = [0 0.1];
yreal = [1e-10 1e-5];
xscale = 'linear';
yscale = 'log10';
% STEP3 the first four data are the plot frame coordinates (x,y) coded as (0,0) (1,0) (1,1) (0,1)
xyframe = XY(1:4,:);   % frame only
xydata  = XY(5:end,:); % data
% STEP4: coordinates transform
if strcmp(xscale,'linear')
    xtransform = @(x) xreal(1) +  diff(xreal)* (x-xyframe(1,1))/diff(xyframe(1:2,1));
elseif strcmp(xscale,'log')
    xtransform = @(x) exp( log(xreal(1)) +  diff(log(xreal))* (x-xyframe(1,1))/diff(xyframe(1:2,1)) );
elseif strcmp(xscale,'log10')
    xtransform = @(x) 10.^( log10(xreal(1)) +  diff(log10(xreal))* (x-xyframe(1,1))/diff(xyframe(1:2,1)) );
else
    error('unknown scale ''%s''',xscale)
end
if strcmp(yscale,'linear')
    ytransform = @(y) yreal(1) +  diff(yreal)* (y-xyframe(1,2))/diff(xyframe([1 end],2));
elseif strcmp(yscale,'log')
    ytransform = @(y) exp( log(yreal(1)) +  diff(log(yreal)) * (y-xyframe(1,2))/diff(xyframe([1 end],2)) );
elseif strcmp(yscale,'log10')
    ytransform = @(y) 10.^( log10(yreal(1)) +  diff(log10(yreal))* (y-xyframe(1,2))/diff(xyframe([1 end],2)) );
else
    error('unknown scale ''%s''',xscale)
end
%STEP5: convert to real coordinates
xyrealdata = [xtransform(xydata(:,1)) ytransform(xydata(:,2))];
%}
