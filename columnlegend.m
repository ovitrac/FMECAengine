function [legend_h,object_h,plot_h,text_strings] = columnlegend(hp, str, numcolumns, varargin)
%
%   columnlegend creates a legend with a specified number of columns.
%   
%   columnlegend(hp, str, numcolumns, varargin)
...
% Modified INRA\Olivier Vitrac - 21/02/16 to set handles, and additional properties

% Default
keywords = {'boxon' 'boxoff'};
default = struct('location','NorthEast','fontsize',10);
[options,remain] = argcheck(varargin,default,keywords);
boxon = options.boxon && ~options.boxoff;
location = options.location;

%create the legend
[legend_h,object_h,plot_h,text_strings] = legend(hp,str,'fontsize',options.fontsize,remain{:});
