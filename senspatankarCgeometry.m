function res = senspatankarCgeometry(F,ploton,dispon)
%SENSPATANKARCGEOMETRY simulates transfer and reactions through n layers (arbitrary coordinates n=0,1,2) using a modified Patankar Method (see p 45)
...
%  R=senspatankarCgeometry(F)
%
% Example: cylindrical
%  F = struct('ngeometry',1,'rinner',0.5,'Bi',1e3,'k',1,'D',1,'k0',1,'l',1,'L',1/20,'C0',1000,'t',[0:.00001:.005 .01:.01:.1 .11:.1:5])
%  R=senspatankarCgeometry(F)
...
Fdefault	= 	struct(...
                'ngeometry' ,   0,... 0=Cartesian, 1=cylindrical, 2=spherical coordinates
                'rinner'    ,   0,... to be used only with ngeometry=1 (inner radius)
				'Bi'		, 	1e3,...	Biot [hm.L1/D]
...
if ~isfield(F,'ngeometry'), F.ngeometry= 0; end
if ~isfield(F,'CF0'), F.CF0 = 0; end
...
% equilibrium value
C0eq = sum(F.CF0+F.lref*F.L.*F.C0)/(1+sum((F.k0./F.k .* F.lref*F.L)));
F.peq = F.k0 * C0eq;
...
CF0 = F.CF0/C0eq; % default initial concentration in F
...
xtotal = F.lrefc(end);
if F.ngeometry==0
    hw0model =     1/( (F.k0/k(1))/(F.Bi)             + dw(1)/(D(1))   );
    hwmodel = @(i) 1/( (de(i-1)/D(i-1))*(k(i-1)/k(i)) + dw(i)/D(i)     );
elseif F.ngeometry==1   % tube (inner part is in contact)
    rmesh = F.rinner + xmesh;
    hw0model =     1/(   (F.k0/k(1))/(F.Bi)   ...
                        + (rmesh(1)-dw(1))/D(i) * log(rmesh(1)/(rmesh(1)-dw(1)))    ...
        );
    hwmodel = @(i) 1/ ( k(i-1)/k(i) * (rmesh(i-1)+de(i-1))/D(i-1) * log((rmesh(i-1)+de(i-1))/rmesh(i-1)) ...
                        + (rmesh(i)-dw(i))/D(i) * log(rmesh(i)/(rmesh(i)-dw(i)))  ....
                      );
elseif F.ngeometry==2    % full sphere / only the external surface is in contact
    % note that - arises from the fact that the flux is defined from xmesh instead of remesh
    rmesh = xtotal-xmesh; % x=0 means external surface and x=xtotal (inner sphere)
    hw0model =     1/(   (F.k0/k(1))/(F.Bi)   ...
                        - (rmesh(1)+dw(1))^2/D(1) * (1/(rmesh(1)+dw(1)) - 1/(rmesh(1)))    ...
        );
    hwmodel = @(i) -1/ ( - k(i-1)/k(i) * (rmesh(i-1)-de(i-1))^2/D(i-1) * (1/(rmesh(i-1)-de(i-1))-1/rmesh(i-1)) ...
                        + (rmesh(i)+dw(i))^2/D(i) * (1/(rmesh(i)+dw(i)) - 1/(rmesh(i)))  ....
                      );
else
    error('unrecognized geometry %d (0=Cartesian, 1=cylindrical, 2=spherical coordinates)',F.ngeometry)
end

% equivalent conductances
he = zeros(F.nmesh,1); hw = he;
hw(1) = hw0model; % pervious contact
for i=2:F.nmesh
    hw(i) = hwmodel(i); %1/( (de(i-1)/D(i-1))*(k(i-1)/k(i)) + dw(i)/D(i)  );
end

% flux continuity
% for i=1:F.nmesh-1, he(i) = hw(i+1); end
