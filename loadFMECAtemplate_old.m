...
for s = sheetnames(2:end)
    for f = fieldbysheet.(s{1})
        ...
        elseif any(strcmpi(f,fieldnames(quantitytype))) % quantity class
            if strcmpi(f,'Thickness') || strcmpi(f,'Density') || strcmpi(f,'Time') || strcmpi(f,'Temperature')
                typetobechecked.(s{1}).(f{1}) = 'noNaNquantity';
            else
...
for isub = 1:nsubstance
    if isub == 1
       dbforsim = fmecaengine('fmecamainfile',{'nlayers',nlayer,'nsteps',nsteps,...
            ...
            'CP',{cellfun(@(f) parser.composition.(f)(isub).value,fieldnames(CPforsim))' .* [parser.system.Density(2:end).value]}});
    else
    ...
    for istep = 1:nsteps
        ...
        dbforsim(k).SMLkgm3 = parser.substances.SML(isub).value * parser.system.Density(1).value;
